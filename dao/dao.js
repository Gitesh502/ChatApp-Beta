class Dao {
  ...
  findOne(filter, populateQuery, projectionQuery, callback) {
    if (populateQuery != null) {
      this.collectionType
        .findOne(filter, projectionQuery)
        .populate(populateQuery)
        .exec(callback);
    } else {
  ...
  findOneAndUpdate(filter, updateObj, options, callback) {
    this.collectionType
      .findOneAndUpdate(filter, updateObj, options)
      .exec(callback);
  }
  /**
   *
  ...
  aggregate(findQuery, callback) {
    this.collectionType.aggregate(findQuery).exec(callback);
  }

  populate(result,populate, callback) {
    this.collectionType.populate(result,populate,callback);
  }
